...
class HorseItems extends StatelessWidget {
  const HorseItems({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Horse Items'),
      ),
      body: SafeArea(
        child: Obx(() {
          return ListView.builder(
              itemCount: controller.horseProducts.length,
              itemBuilder: (BuildContext context, int index) {
                var product = controller.horseProducts[index];
